...
(function(){
  ...
  function syncTempoAcrossInputs(rawTempo, options){
    const tempo = clampChallengeTempo(rawTempo);
    if (typeof setMetronomeTempo === 'function') {
      setMetronomeTempo(tempo, options);
    } else {
      const headerInput = document.getElementById('headerMetronomeBpm');
  ...
  function openChallengeModal(){
    ...
    if (bpm) {
      syncTempoAcrossInputs(getUnifiedTempoFromHeader(), { silent: true });
    }
  ...
  async function confirmChallengeSetup(){
    ...
    const bpm = syncTempoAcrossInputs(getChallengeTempoValue(), { silent: true });
  ...
  async function startChallenge(prepSec, bpm, cursorEnabled, metronomeEnabled, hideEnabled){
    ...
    if (typeof setMetronomeTempo === 'function') {
      setMetronomeTempo(bpm, { silent: true });
    } else {
      const headerInput = document.getElementById('headerMetronomeBpm');
  ...
  if (challengeBpmEl){
    const onChallengeBpmChanged = () => {
      ...
      const tempo = syncTempoAcrossInputs(challengeBpmEl.value, { silent: true });
