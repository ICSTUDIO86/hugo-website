    ...
    try {
      ...
      if (refundCheckResult.data.length > 0) {
        ...
        if (refundRecord.status === 'refunded') {
          await recordAccessAttempt(inputCode, deviceId, 'refunded', '访问码已退款，权限已失效', clientIP, userAgent)
          
          console.log(`[退款检查] 访问码 ${formattedCode} 已退款，退款时间: ${refundRecord.refund_time}`)
          
          return {
            success: false,
            message: '访问码已退款，权限已失效。如有疑问请联系客服。',
            refundInfo: {
              refunded: true,
      ...
      console.warn(`[退款检查] 检查退款状态失败:`, refundError)
      // 退款检查失败时继续验证流程，但记录警告
