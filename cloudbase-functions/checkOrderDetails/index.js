...
exports.main = async (event, context) => {
    ...
    try {
        ...
        let query;
        if (access_code) {
            // 优先根据访问码查询 ic_studio_orders 集合
            query = db.collection('ic_studio_orders').where({ access_code: access_code });
        } else if (order_no) {
            // 根据订单号查询
            query = db.collection('ic_studio_orders').where({ out_trade_no: order_no });
        } else {
            // 查询最近的订单
            query = db.collection('ic_studio_orders')
