...
function initializeAudio() {
    ...
    try {
        ...
        lowPassFilter.frequency.value = 2000; // 截止频率2000Hz，过滤掉刺耳的高频
        lowPassFilter.Q.value = 1; // 滤波器质量因子

        // 连接音频处理链：oscillator -> lowpass filter -> master gain -> destination
        lowPassFilter.connect(masterGain);
        masterGain.connect(audioContext.destination);

        isAudioInitialized = true;
        console.log('🎵 音频系统已初始化，包含低通滤波器（截止频率: 2000Hz）');
    } catch (error) {
...
function playNote(midiNote, duration = 0.5) {
    ...
    try {
        ...
        noteGain.gain.linearRampToValueAtTime(0.8, now + 0.05); // 快速攻击
        noteGain.gain.exponentialRampToValueAtTime(0.01, now + duration); // 慢衰减

        // 🔥 关键：通过低通滤波器来过滤高频
        oscillator.connect(noteGain);
        noteGain.connect(lowPassFilter); // 连接到低通滤波器

        oscillator.start(now);
        oscillator.stop(now + duration);

        console.log(`🎵 播放音符: MIDI ${midiNote} (${frequency.toFixed(1)}Hz) - 经过低通滤波`);
    } catch (error) {
...
async function playMelody() {
    ...
    // 恢复AudioContext（某些浏览器需要用户交互才能启动）
...
function adjustAudioFilter(cutoffFreq = 2000, volume = 0.3) {
    ...
    if (lowPassFilter) {
        ...
        console.log(`🎛️ 低通滤波器截止频率已调整为: ${cutoffFreq}Hz`);
    }
...
function setAudioPreset(preset) {
    switch (preset) {
        case 'warm': // 温暖音色
            ...
            console.log('🎵 已设置为温暖音色（低通1500Hz）');
            break;
        case 'bright': // 明亮音色
            adjustAudioFilter(3000, 0.3);
            console.log('🎵 已设置为明亮音色（低通3000Hz）');
            break;
        case 'soft': // 柔和音色
            adjustAudioFilter(1200, 0.2);
            console.log('🎵 已设置为柔和音色（低通1200Hz）');
            break;
        default:
            adjustAudioFilter(2000, 0.3);
            console.log('🎵 已设置为默认音色（低通2000Hz）');
    }
}
...
const BEAMING_RULES = {
    ...
    basicRules: {
        ...
        // 符槓类型对应
        beamLevels: {
            'quarter': 1,   // 四分音符：1条符槓（在特殊情况下）
            'quarter.': 1,  // 附点四分音符：1条符槓
            'eighth': 1,    // 八分音符：1条符槓
...
const RHYTHM_NOTATION_RULES = {
    ...
    // 🎵 动态细分边界计算：根据附点音符调整拍点细分
    getDynamicSubdivisionBoundaries: function(timeSignature, allowedRhythms) {
        // 🔥 修复：使用正确的对象引用
        const rules = this.timeSignatureRules[timeSignature];
        if (!rules) {
            console.warn(`⚠️ 未找到拍号${timeSignature}的规则，使用默认细分`);
            return [0, 0.5, 1, 1.5, 2, 2.5, 3, 3.5]; // 默认八分音符细分
        }

        let subdivisions = [...rules.subdivisionBoundaries]; // 复制默认边界

        if (timeSignature === '4/4' && Array.isArray(allowedRhythms)) {
            // 🎵 根据附点音符调整4/4拍的细分规则
            const hasDottedHalf = allowedRhythms.includes('half.');      // 附点二分音符
            const hasDottedQuarter = allowedRhythms.includes('quarter.'); // 附点四分音符
            const hasDottedEighth = allowedRhythms.includes('eighth.');   // 附点八分音符

            if (hasDottedEighth) {
                // 附点八分音符存在时，按十六分音符细分
                subdivisions = [0, 0.25, 0.5, 0.75, 1, 1.25, 1.5, 1.75, 2, 2.25, 2.5, 2.75, 3, 3.25, 3.5, 3.75];
                console.log(`🎵 检测到附点八分音符，调整为十六分音符细分`);
            } else if (hasDottedQuarter) {
                // 附点四分音符存在时，按八分音符细分（默认）
                subdivisions = [0, 0.5, 1, 1.5, 2, 2.5, 3, 3.5];
                console.log(`🎵 检测到附点四分音符，使用八分音符细分`);
            } else if (hasDottedHalf) {
                // 仅附点二分音符存在时，按四分音符细分
                subdivisions = [0, 1, 2, 3];
                console.log(`🎵 检测到附点二分音符，调整为四分音符细分`);
            }
        }
...
class IntelligentMelodyGenerator {
    ...
    getTimeSignatureAppropriateRhythms(timeSignature, userAllowedRhythms) {
        ...
        // 🔥 修复：userSettings.allowedRhythms已经存储内部格式（quarter.等），无需转换
        console.log(`🎵 ${timeSignature}拍转换后的节奏（包括附点音符）: [${baseRhythms.join(', ')}]`);
    ...
    generateMeasure(measureIndex, lastMidi, lastDirection, consecutiveJumps) {
        ...
        while (remainingBeats > 0.001 && loopSafety < maxLoopIterations) {
            ...
            if (beats > remainingBeats + 0.001) {
                ...
                if (restBeatsDifference > 0.001) {
                    console.log(`🔧 [分数拍修复] 检测到休止符时值不匹配：需要${safeRestBeats}拍，但${proposedRestDuration}实际是${actualRestBeats}拍`);
                    ...
                    for (const segment of restSegments) {
                        ...
                        console.log(`✅ 添加拆分休止符: ${segmentDuration} (${segment}拍)`);
                    }
                    ...
                    console.log(`✅ 添加填充休止符: ${proposedRestDuration} (${safeRestBeats}拍)`);
                    remainingBeats -= safeRestBeats;
                    currentBeat += safeRestBeats;
                }
        ...
        if (Math.abs(beatsDifference) > 0.001) {
            ...
            if (beatsDifference > 0) {
                ...
                if (restBeatsDifference > 0.001) {
                    console.log(`🔧 [最终验证-分数拍修复] 检测到时值不匹配：需要${beatsDifference}拍，但${proposedRestDuration}实际是${actualRestBeats}拍`);
                    ...
                    for (const segment of restSegments) {
                        ...
                        console.log(`✅ 添加最终验证拆分休止符: ${segmentDuration} (${segment}拍)`);
                        totalBeats += segment;
                    }
                } else {
                    // 标准处理：单个休止符
            ...
            if (Math.abs(finalBeatsDifference) > 0.001) {
                ...
                if (finalBeatsDifference > 0) {
                    ...
                    if (finalRestBeatsDifference > 0.001) {
                        console.log(`🔧 [最终救救填充-分数拍修复] 检测到时值不匹配：需要${finalBeatsDifference}拍，但${proposedFinalRestDuration}实际是${actualFinalRestBeats}拍`);
                        ...
                        for (const segment of finalRestSegments) {
                            ...
                            console.log(`✅ 添加最终救救填充拆分休止符: ${segmentDuration} (${segment}拍)`);
                        }
                    } else {
                        // 标准处理：单个休止符
                        notes.push({
        ...
        const weights = availablePatterns.map(pattern => {
            ...
            if (userSettings && userSettings.rhythmFrequencies) {
                ...
                for (const duration of uniqueDurations) {
                    // 🔥 修复：正确映射duration到频率键名
                    let freqKey = duration;
                    if (duration === 'half.') {
                        // 6/8拍的附点二分音符使用全音符频率（整小节）
                        freqKey = 'whole';
                    } else if (duration === 'quarter.') {
                        // 🔥 修复：6/8拍的附点四分音符使用自己的频率设置
                    ...
                    if (userFreq !== undefined) {
                        if (userFreq === 0) {
                            console.log(`🚫 6/8拍模式包含频率为0%的节奏类型: ${duration} (映射到${freqKey})`);
                ...
                // 🔥 修复：直接使用平均频率作为权重，不再作为乘数
                if (validFrequencyCount > 0) {
                    const averageFrequency = totalUserFrequency / validFrequencyCount;
                    console.log(`🎯 6/8拍模式平均频率权重: ${averageFrequency.toFixed(1)}% (节奏: ${uniqueDurations.join(', ')})`);
                    // 直接返回平均频率作为权重
            ...
            if (durations.every(d => d === 'eighth') && durations.length === 6) {
                ...
                baseWeight = 10; // 清晰的3+3分组
            } else if (durations.includes('quarter.')) {
                baseWeight = 10; // 其他附点四分音符混合模式
            } else if (durations.includes('quarter')) {
                baseWeight = 10; // 包含四分音符的模式
            }

            console.log(`🎵 6/8拍模式默认权重: [${durations.join(' ')}] = ${baseWeight}`);

            return Math.max(baseWeight, 0); // 确保权重不为负数
        });
    ...
    chooseDuration(remainingBeats, isFirstNote, currentBeat = 0) {
            ...
            if (this.timeSignature === '4/4' || this.timeSignature === '2/4' || this.timeSignature === '3/4') {
                ...
                if (duration === 'eighth.' && beats === 0.75) {
                    ...
                    if (this.timeSignature === '4/4') {
                        ...
                        quarterBeatPositions = [0, 1, 2, 3]; // 默认情况
                    }
                    ...
                    if (remainingAfterDotted > 0.001 && remainingAfterDotted < 0.24) {
                        console.log(`🚫 附点八分音符被阻止：剩余空间${remainingAfterDotted}过小，难以填充`);
                        return false;
                    }

                    // 检查2：确保剩余拍数能够被精确填充（避免分数拍问题）
                    if (remainingAfterDotted > 0.001) {
                        ...
                        if (remainingDifference > 0.001) {
                            console.log(`🚫 附点八分音符被阻止：剩余${remainingAfterDotted}拍无法精确填充，差值${remainingDifference}拍`);
                            return false;
                        }
                    }

                    // 检查3：避免在小节接近末尾时使用附点八分音符
                    ...
                    console.log(`✅ 附点八分音符前置检查通过：位置${currentBeat}，剩余${remainingAfterDotted}拍可精确填充`);
                }

                // 附点四分音符 (1.5拍) 拍点对齐检查
                if (duration === 'quarter.' && beats === 1.5) {
                    // 重新定义四分音符拍点位置（每个拍号独立处理）
                    let quarterBeatPositions;
                    if (this.timeSignature === '4/4') {
                        quarterBeatPositions = [0, 1, 2, 3];
                    } else if (this.timeSignature === '3/4') {
                ...
                // 🔥 关键修复：普通二分音符(2拍)必须在二分音符拍点开始
                if (duration === 'half' && beats === 2) {
                    let halfBeatPositions;
                    if (this.timeSignature === '4/4') {
                        halfBeatPositions = [0, 2]; // 4/4拍的二分音符拍点
                    } else if (this.timeSignature === '3/4') {
                        halfBeatPositions = [0]; // 3/4拍只能在开始
                    } else if (this.timeSignature === '2/4') {
                        halfBeatPositions = [0]; // 2/4拍只能在开始
                    } else {
                        halfBeatPositions = [0, 2]; // 默认情况
                    }
                ...
                if (duration === 'eighth' && beats === 0.5) {
                    ...
                    if (this.timeSignature === '4/4') {
                        ...
                        eighthBeatPositions = [0, 0.5, 1, 1.5, 2, 2.5, 3, 3.5]; // 默认4/4拍
                    }
        ...
        // 🎯 4/4拍八分音符配对逻辑：如果上一个音符需要配对，且剩余0.5拍，优先选择八分音符
        if (this.timeSignature === '4/4' && this._expectEighthNotePair && Math.abs(remainingBeats - 0.5) < 0.001 && available.includes('eighth')) {
            ...
            if (eighthFreq > 0) {
                console.log(`🎯 [八分音符配对] 完成配对：剩余0.5拍，选择八分音符与上一个配对（用户频率: ${eighthFreq}%）`);
                this._expectEighthNotePair = false; // 重置标记
                return 'eighth';
            }
        }

        // 🎵 智能附点音符优先级：在合适的位置优先考虑附点音符
        if (this.timeSignature === '4/4' || this.timeSignature === '2/4' || this.timeSignature === '3/4') {
            const tolerance = 0.001;

            // 在四分音符拍点且剩余至少1拍时，考虑附点八分音符
            if (Math.abs(currentBeat % 1) < tolerance && remainingBeats >= 0.75) {
                if (available.includes('eighth.') && this.random.nextFloat() < 0.4) { // 40%概率优先选择
                    const dottedEighthFreq = getUserFrequency('rhythm', 'dotted-eighth');
                    if (dottedEighthFreq > 0) {
                        console.log(`🎵 智能优先：在四分音符拍点${currentBeat}选择附点八分音符`);
                        return 'eighth.';
                    }
                }
            }

            // 在强拍且剩余至少2拍时，考虑附点四分音符
            if ((Math.abs(currentBeat - 0) < tolerance || Math.abs(currentBeat - 2) < tolerance) && remainingBeats >= 1.5) {
                if (available.includes('quarter.') && this.random.nextFloat() < 0.3) { // 30%概率优先选择
                    const dottedQuarterFreq = getUserFrequency('rhythm', 'dotted-quarter');
                    if (dottedQuarterFreq > 0) {
                        console.log(`🎵 智能优先：在强拍${currentBeat}选择附点四分音符`);
                        return 'quarter.';
                    }
                }
            }
                ...
                if (this.timeSignature === '4/4' && Math.abs(remainingBeats - 1) < 0.001 && available.includes('eighth')) {
                    ...
                    if (eighthFreq > 0) {
                        ...
                        if (isOnQuarterBeat) {
                            ...
                            console.log(`🎯 4/4拍八分音符配对规则：位置${currentBeat}剩余1拍，选择八分音符促进连杆（用户频率: ${eighthFreq}%）`);
                        }
                    }
                }
            }
        ...
        if (remainingAfterNote > 0.001) {
            // 确保剩余空间至少能放下一个十六分音符或合理的休止符
            if (remainingAfterNote < 0.24 && remainingAfterNote > 0.001) {
                console.log(`⚠️ 时值验证：选择${selectedDuration}(${finalBeats}拍)后剩余${remainingAfterNote}拍，空间过小难以填充`);

                // 尝试选择更小的时值以留出合理空间
                const saferOptions = available.filter(d => {
                    const dBeats = this.durationToBeats(d);
                    const wouldRemain = remainingBeats - dBeats;
                    return wouldRemain === 0 || wouldRemain >= 0.25; // 要么正好填满，要么至少留下十六分音符的空间
                });
                ...
                if (saferOptions.length > 0) {
                    ...
                    if (perfectFit) {
                        ...
                        console.log(`✅ 选择完美匹配时值: ${selectedDuration}`);
                    } else {
                        // 否则选择最大的安全时值
                        selectedDuration = saferOptions.reduce((best, current) =>
                            this.durationToBeats(current) > this.durationToBeats(best) ? current : best
                        );
                        console.log(`✅ 选择更安全的时值: ${selectedDuration}`);
                    }
                }
            }
        }

        console.log(`🎯 最终选择的节奏时值: ${selectedDuration}(${this.durationToBeats(selectedDuration)}拍)`);

        // 重置八分音符配对标记（当选择的不是八分音符时）
    ...
    generateBeams(notes, currentBeatLevel = null) {
        ...
        // 根据拍号选择合适的beaming方法
        if (timeSignature === '4/4') {
            console.log(`使用4/4拍专用beaming逻辑`);
            return this.generateBeamsFor4_4(notes);
        } else if (timeSignature === '3/4') {
            console.log(`使用3/4拍专用beaming逻辑`);
    ...
    /**
     * 4/4拍超级简单测试方法 - 直接连接所有可连杆音符
     */
    generateBeamsFor4_4(notes) {
        console.log(`🔥 使用超级简单的beaming逻辑 - 直接连接所有可连杆音符`);
        console.log(`🔍 传入的音符数据总数: ${notes.length}`);

        // 详细打印所有音符数据
        notes.forEach((note, i) => {
            console.log(`  音符${i}: type=${note.type}, duration=${note.duration}, beats=${note.beats}, pitch=${note.pitch || '无'}`);
        });
        ...
        for (let i = 0; i < notes.length; i++) {
            const note = notes[i];
            const beatNumber = Math.floor(position) % 4;

            console.log(`🔍 处理音符${i}: ${note.type}/${note.duration}, 拍${beatNumber}, 位置${position.toFixed(3)}`);

            // 最简单的连杆检查
            let canBeam = false;
            if (note.type === 'note') {
                // 只要不是全音符或二分音符，都可以连杆
                canBeam = note.duration !== 'whole' && note.duration !== 'half' && note.duration !== 'half.';
                console.log(`  🔍 音符${i}(${note.duration}): 可连杆=${canBeam}`);
            }
            ...
            if (canBeam) {
                ...
                if (currentBeat !== -1 && beatNumber !== currentBeat) {
                    ...
                    if (currentGroup.length >= 2) {
                        console.log(`    ✅ 创建连杆组: 音符[${currentGroup.map(idx => idx+1).join(', ')}]`);
                        beamGroups.push({
                            start: currentGroup[0],
                            end: currentGroup[currentGroup.length - 1],
                            notes: [...currentGroup],
                            beamLevels: [[...currentGroup]], // 简单的beam级别
                            stemDirection: 'up'
                        });
                    }
                    currentGroup = [];
                ...
                currentGroup.push(i);
                console.log(`  ✅ 加入拍${beatNumber}连杆组，组大小: ${currentGroup.length}`);
        ...
        // 处理最后一组
        if (currentGroup.length >= 2) {
            console.log(`  ✅ 创建最终连杆组: 音符[${currentGroup.map(idx => idx+1).join(', ')}]`);
            beamGroups.push({
                start: currentGroup[0],
                end: currentGroup[currentGroup.length - 1],
                notes: [...currentGroup],
        ...
        console.log(`🔥 超简单beaming完成: 共${beamGroups.length}个beam组`);
        beamGroups.forEach((group, i) => {
            console.log(`  连杆组${i+1}: 音符[${group.notes.map(n => n+1).join(',')}]`);
        });
        return beamGroups;
    }
    ...
    generateBeamsFor4_4_Complex(notes) {
        console.log(`🎵 使用4/4拍专用beam生成逻辑 - 严格按拍点分组`);

        const beamGroups = [];
        let currentPosition = 0;

        // 将音符按照严格的拍点分组 - 第1拍[0-1), 第2拍[1-2), 第3拍[2-3), 第4拍[3-4)
        const beatGroups = [[], [], [], []]; // 4个四分音符拍
        ...
        for (let i = 0; i < notes.length; i++) {
            ...
            console.log(`🎵 分析音符${i+1}: ${note.duration}, 绝对位置${noteStart.toFixed(3)}-${noteEnd.toFixed(3)}, 小节内位置${measurePosition.toFixed(3)}-${(measureEnd || 4).toFixed(3)}`);

            // 简化的边界检查：如果音符跨越任何整数拍点边界，则不能beam
            let crossesBeat = false;

            // 检查是否跨越任何拍点边界 (1.0, 2.0, 3.0)
        ...
        for (let beatIndex = 0; beatIndex < 4; beatIndex++) {
            ...
            if (beatNotes.length >= 2) {
                ...
                console.log(`  🔍 拍${beatIndex+1}有十六分音符: ${hasSixteenth}`);
                ...
                for (let i = 0; i < beatNotes.length; i++) {
                    ...
                    // 直接检查音符是否可连杆 - 使用最简单的规则
                    let canBeBeamed = false;
                    if (note.type === 'note') {
                        if (hasSixteenth) {
                            // 有十六分音符时：四分、八分、十六分都可连杆
                        ...
                        console.log(`    🔍 音符${noteIndex+1}(${note.duration}): 可连杆=${canBeBeamed}, 有十六分=${hasSixteenth}`);
                    }
        ...
        console.log(`🎵 4/4拍复杂beam生成完成: 共${beamGroups.length}个beam组`);
        return beamGroups;
    }
    ...
    forceBeamsFor44(notes) {
        ...
        for (let i = 0; i < notes.length; i++) {
            ...
            // 修复浮点数精度问题：四舍五入到小数点后4位，然后取整
            const cleanPosition = Math.round(position * 10000) / 10000;
            const noteBeat = Math.floor(cleanPosition) % 4; // 确保拍号在0-3范围内
            const beatInfo = beatAnalysis.get(noteBeat);

            console.log(`🔍 处理音符${i}: ${note.type}/${note.duration}, 拍${noteBeat}, 位置${position.toFixed(4)} -> 清理后${cleanPosition.toFixed(4)}`);

            // 强制小节边界检查 - 如果跨越小节，立即终止当前连杆组
            const measureNumber = Math.floor(cleanPosition / 4);
            const beatInMeasure = noteBeat; // noteBeat已经是0-3范围内的拍号
            const currentMeasure = currentBeat !== -1 ? Math.floor((position - note.beats) / 4) : measureNumber;

            // 调试：特别关注第3和第4拍的连杆问题
            if (noteBeat >= 2) {
                console.log(`  🎯 [第${noteBeat+1}拍调试] 音符${i}: ${note.duration}, 原始位置: ${position.toFixed(6)}, 清理位置: ${cleanPosition.toFixed(6)}, 拍: ${noteBeat}`);
                console.log(`  🎯 [第${noteBeat+1}拍调试] 当前连杆组大小: ${currentGroup.length}, 当前拍: ${currentBeat}`);
                console.log(`  🎯 [第${noteBeat+1}拍调试] 音符类型: ${note.type}, 是否八分音符: ${note.duration === 'eighth'}`);
            }

            if (currentGroup.length > 0 && measureNumber !== currentMeasure) {
                console.log(`  🚫 检测到小节边界（小节${currentMeasure}→${measureNumber}），强制终止连杆组`);
            ...
            if (note.type === 'rest') {
                ...
                if (shouldBeamAllInBeat) {
                    ...
                    console.log(`  📏 拍${noteBeat}标准连杆规则: ${shouldBeam ? '连杆' : '不连杆'}`);

                    // 第4拍调试：详细检查isStandardBeamable的结果
                    if (noteBeat === 3) {
                        console.log(`  🎯 [第4拍标准连杆] 音符${i}: type=${note.type}, duration=${note.duration}, beats=${note.beats}, shouldBeam=${shouldBeam}`);
                    }
                }
            }
            ...
            if (shouldBeam) {
                ...
                if (shouldTerminateGroup) {
                    ...
                    console.log(`  📍 强制结束连杆组 - 换拍: ${reason1}, 跨小节: ${reason2}, 当前拍: ${currentBeat}→${noteBeat}, 位置: ${cleanPosition.toFixed(4)}`);

                    // 第4拍调试
                    if (noteBeat === 3) {
                        console.log(`  🚨 [第4拍终止] 第4拍连杆组被意外终止! 原因: 换拍=${reason1}, 跨小节=${reason2}`);
                        console.log(`  🚨 [第4拍终止] 当前组大小: ${currentGroup.length}, currentBeat: ${currentBeat}, noteBeat: ${noteBeat}`);
                    }
                ...
                // 第4拍调试
                if (noteBeat === 3) {
                    console.log(`  🎯 [第4拍连杆] 成功加入第4拍连杆组: 音符${i} (${note.duration}), 组大小: ${currentGroup.length}`);
                }
            } else {
                console.log(`  ❌ 不连杆/中断连杆组`);
                ...
                // 第4拍调试
                if (noteBeat === 3) {
                    console.log(`  🚨 [第4拍警告] 第4拍音符${i} (${note.duration}) 被拒绝连杆! shouldBeam=${shouldBeam}`);
                }
    ...
    analyzeBeatContents(notes) {
        ...
        for (let i = 0; i < notes.length; i++) {
            ...
            // 修复浮点数精度问题：四舍五入到小数点后4位，然后取整
            const cleanPosition = Math.round(position * 10000) / 10000;
            const beatNumber = Math.floor(cleanPosition) % 4; // 确保拍号在0-3范围内
    ...
    isStandardBeamable(note) {
        ...
        console.log(`    📏 标准连杆检查: duration=${note.duration}, beats=${note.beats}, tied=${note.tied}, tieType=${note.tieType}`);
        ...
        console.log(`    → 详细检查: eighth=${isEighth}, eighth.=${isDottedEighth}, 16th=${isSixteenth}, 16th.=${isDottedSixteenth}, 32nd=${isThirtySecond}`);
    ...
    get TRIPLET_RULES() {
        ...
        placementRules: {
            ...
            canPlaceTriplet: function(position, tripletType, timeSignature, remainingBeats) {
                ...
                // 这个函数在IntelligentMelodyGenerator的上下文中被调用
                // 需要通过适当的路径访问types
                const types = {
                    // 三连音
    ...
    beatsToNoteDuration(beats) {
        ...
        // 🔥 附点八分音符精确处理：检测常见的分数拍并返回最接近的标准时值
        const preciseTolerances = [
            ...
            { beats: 0.125, duration: '16th' }      // 0.125拍（十六分音符）
        ];
    ...
    splitFractionalBeats(beats) {
        console.log(`🔧 [分数拍拆分] 开始拆分${beats}拍`);
        ...
        while (remaining > tolerance) {
            ...
            for (const beatValue of standardBeats) {
                ...
                if (beatValue === 0.25 && !this.rules.allowedDurations.includes('16th')) {
                    continue; // 跳过16分音符如果用户不允许
                }

                if (remaining >= beatValue - tolerance) {
                    segments.push(beatValue);
                    remaining -= beatValue;
                    console.log(`  ✅ 添加${beatValue}拍段，剩余${remaining.toFixed(3)}拍`);
                    foundMatch = true;
                    break;
                }
            }

            // 如果没有找到匹配的标准时值，强制使用最小允许的时值
            if (!foundMatch) {
                const minBeat = this.rules.allowedDurations.includes('16th') ? 0.25 : 0.5;
                if (remaining > tolerance) {
                    segments.push(minBeat);
                    remaining -= minBeat;
                    console.log(`  ⚠️ 强制添加最小时值${minBeat}拍，剩余${remaining.toFixed(3)}拍`);
                } else {
                    break; // 剩余拍数太小，忽略
                }
            }
            ...
            if (segments.length > 10) {
                console.warn(`⚠️ 拆分过程中检测到可能的无限循环，停止拆分`);
                break;
            }
        }

        console.log(`🔧 [分数拍拆分] 完成：${beats}拍 -> [${segments.join(', ')}]拍`);
        return segments;
    }
...
class MusicXMLBuilder {
    ...
    /**
     * 🔥 在四分音符合并后重新生成beaming信息
     */
    regenerateBeamingAfterMerge(melody, timeSignature) {
        console.log('🔥 开始重新生成beaming信息...');

        for (let measureIndex = 0; measureIndex < melody.length; measureIndex++) {
            const measure = melody[measureIndex];
            if (!measure || !measure.notes) continue;

            console.log(`🔥 处理小节${measureIndex + 1}: ${measure.notes.length}个音符`);

            // 创建一个临时的NotationGenerator来重新生成beaming
            const tempGenerator = {
                timeSignature: timeSignature,
                generateBeams: function(notes) {
                    // 使用与IntelligentMelodyGenerator相同的beaming逻辑
                    console.log(`🎯 重新生成beaming: ${notes.length}个音符`);
                ...
                forceBeamsFor44: function(notes) {
                    console.log(`🔥 使用修复后的beaming重新生成逻辑`);
                    console.log(`🔍 处理${notes.length}个音符:`, notes.map((n, i) => `${i}:${n.duration}`).join(', '));
                    const beamGroups = [];
                    let currentGroup = [];
                    let position = 0;
                    let currentBeat = -1;
                    ...
                    for (let i = 0; i < notes.length; i++) {
                        ...
                        console.log(`  🔍 检查音符${i}: ${note.type}/${note.duration}, 拍${noteBeat}, 位置${position.toFixed(4)}`);

                        // 处理所有可连杆音符（八分、十六分、四分等）
                        const canBeam = note.type === 'note' && (
                        ...
                        if (canBeam) {
                            console.log(`    🔍 音符${i}(${note.duration})可以连杆`);

                            // 检查是否需要开始新组
                            if (currentBeat !== -1 && noteBeat !== currentBeat) {
                                // 结束当前组
                                if (currentGroup.length >= 2) {
                                    console.log(`    ✅ 创建连杆组: 拍${currentBeat}, 音符[${currentGroup.join(',')}]`);
                                    beamGroups.push({
                                        start: currentGroup[0],
                                        end: currentGroup[currentGroup.length - 1],
                                        notes: [...currentGroup],
                            ...
                            // 不可连杆音符（休止符或长音符），结束当前组
                            console.log(`    ❌ 音符${i}(${note.type}/${note.duration})不能连杆`);
                            if (currentGroup.length >= 2) {
                                console.log(`    ✅ 遇到不可连杆音符，创建连杆组: 拍${currentBeat}, 音符[${currentGroup.join(',')}]`);
                                beamGroups.push({
                                    start: currentGroup[0],
                                    end: currentGroup[currentGroup.length - 1],
                                    notes: [...currentGroup],
                    ...
                    // 处理最后一组
                    if (currentGroup.length >= 2) {
                        console.log(`    ✅ 创建最后连杆组: 拍${currentBeat}, 音符[${currentGroup.join(',')}]`);
                        beamGroups.push({
                            start: currentGroup[0],
                            end: currentGroup[currentGroup.length - 1],
                            notes: [...currentGroup],
                    ...
                    console.log(`🔥 重新生成的beaming结果: ${beamGroups.length}个连杆组`);
                    return beamGroups;
                }
            };
            ...
            console.log(`🔥 小节${measureIndex + 1}更新后: ${measure.beams.length}个连杆组`);
        }

        console.log('🔥 beaming重新生成完成!');
        return melody;
    }
...
/**
 * 🎵 合并4/4拍中的连线四分音符
 * 最高权限规则：在4/4拍中，如果两个四分音符通过tie连接，并且都在四分音符拍点上，必须合并
 * @param {Array} melody - 旋律数组
 * @param {string} timeSignature - 拍号
 * @returns {Array} - 处理后的旋律
 */
function mergeTiedQuarterNotesIn44(melody, timeSignature) {
    ...
    console.log('🎼 === 4/4拍连线四分音符合并处理 ===');
    let mergeCount = 0;

    // 🎯 简化助手函数：只检查真正必要的冲突，默认允许合并
    function shouldMergeBasedOnContext(notes, currentIndex, measureIndex) {
        ...
        console.log(`  🎯 [简化检查] 小节${measureIndex + 1}: 位置${firstNotePos.toFixed(2)}-${secondNotePos.toFixed(2)}, 拍${beatOfFirst}-${beatOfSecond}`);

        // 规割1：如果在同一拍内且各占0.5拍，应该保持为八分音符beaming
        if (beatOfFirst === beatOfSecond &&
            Math.abs(currentNote.beats - 0.5) < 0.001 &&
            Math.abs(nextNote.beats - 0.5) < 0.001) {
            console.log(`  ⚠️ 同一拍内的0.5拍 tied音符，保持beaming结构`);
            return false;
        }

        // 规割2：对于相邻拍的标准四分音符，默认允许合并
        if (Math.abs(beatOfSecond - beatOfFirst - 1) < 0.001 &&
            ...
            console.log(`  ✅ 相邻拍的标准四分音符，允许合并`);
            return true;
        }

        // 其他情况保守处理
        console.log(`  ⚠️ 不符合标准情况，保守处理`);
        return false;
    }
    ...
    for (let measureIndex = 0; measureIndex < melody.length; measureIndex++) {
        ...
        while (i < measure.notes.length) {
            ...
            if (currentNote && nextNote &&
                ...
                // 🎯 智能分析：基于上下文决定是否合并
                const shouldMerge = shouldMergeBasedOnContext(measure.notes, i, measureIndex);
                if (!shouldMerge) {
                    console.log(`  🎵 小节${measureIndex + 1}: 基于上下文分析，不合并这两个音符`);
                ...
                if (isFirstOnBeat && isSecondOnBeat) {
                    console.log(`  ✅ 小节${measureIndex + 1}: 合并位置${firstNotePosition.toFixed(2)}和${secondNotePosition.toFixed(2)}的四分音符`);
    ...
    if (mergeCount > 0) {
        console.log(`🎵 共合并了 ${mergeCount} 组连线四分音符为二分音符`);
    }
...
function generateMelodyData(measures, keySignature, timeSignature, clef, seed = null) {
    ...
    if (timeSignature === '6/8') {
        ...
        // 6/8拍：移除三连音，保留二连音和四连音
        const filtered68Rhythms = originalAllowedRhythms.filter(rhythm => rhythm !== 'triplet');
        console.log(`🎵 6/8拍连音过滤: 移除triplet，保留duplet/quadruplet`);
        console.log(`   原始: [${originalAllowedRhythms.join(', ')}]`);
        console.log(`   过滤后: [${filtered68Rhythms.join(', ')}]`);
        ...
        function generateSmartRhythmPattern() {
            ...
            if (availableRhythms.includes('eighth')) {
                ...
                const eighthFreq = (userSettings && userSettings.rhythmFrequencies && userSettings.rhythmFrequencies.eighth !== undefined) ?
                    userSettings.rhythmFrequencies.eighth : 40; // 默认40%

                console.log(`🎯 6/8拍八分音符频率设置: ${eighthFreq}%`);

                if (eighthFreq > 0) {
                    patterns.push({
                        name: 'sixEighths',
                        weight: eighthFreq, // 🔥 修复：直接使用频率作为权重
                        pattern: [
                            { duration: 'eighth', beats: 0.5 },
                            { duration: 'eighth', beats: 0.5 },
                            { duration: 'eighth', beats: 0.5 },
                    ...
                    console.log(`🎵 添加六个八分音符模式，权重: ${eighthFreq}`);
                } else {
                    console.log(`🚫 八分音符频率为0%，跳过八分音符模式`);
                }
            }
            ...
            if (availableRhythms.includes('quarter.')) {
                ...
                const dottedQuarterFreq = (userSettings && userSettings.rhythmFrequencies && userSettings.rhythmFrequencies['dotted-quarter'] !== undefined) ?
                    userSettings.rhythmFrequencies['dotted-quarter'] : 35; // 默认35%

                console.log(`🎯 6/8拍附点四分音符频率设置: ${dottedQuarterFreq}%`);

                if (dottedQuarterFreq > 0) {
                    patterns.push({
                        name: 'twoDotted',
                        weight: dottedQuarterFreq, // 🔥 修复：直接使用频率作为权重
                        pattern: [
                            { duration: 'quarter', dots: 1, beats: 1.5 },
                            { duration: 'quarter', dots: 1, beats: 1.5 }
                        ]
                    });
                    console.log(`🎵 添加两个附点四分音符模式，权重: ${dottedQuarterFreq}`);
                } else {
                    console.log(`🚫 附点四分音符频率为0%，跳过附点四分音符模式`);
                }
            }
            
            // 🔥 附点二分音符模式（如果用户允许）- 整小节持续音符
            if (availableRhythms.includes('half.') || availableRhythms.includes('dotted-half')) {
                // 🔥 修复：使用用户频率设置（映射到全音符频率）
                const dottedHalfFreq = (userSettings && userSettings.rhythmFrequencies && userSettings.rhythmFrequencies.whole !== undefined) ?
                    userSettings.rhythmFrequencies.whole : 10; // 默认10%

                console.log(`🎯 6/8拍附点二分音符频率设置(映射到全音符): ${dottedHalfFreq}%`);

                if (dottedHalfFreq > 0) {
                    patterns.push({
                        name: 'dottedHalf',
                        weight: dottedHalfFreq, // 🔥 修复：直接使用频率作为权重
                        pattern: [
                            { duration: 'half', dots: 1, beats: 3.0 }
                        ]
                    });
                    console.log(`🎵 添加附点二分音符模式，权重: ${dottedHalfFreq}`);
                } else {
                    console.log(`🚫 附点二分音符频率为0%，跳过附点二分音符模式`);
                }
            }
            
            // 十六分音符模式（如果用户允许）- 🔥 修复：使用用户频率设置
            if (availableRhythms.includes('16th')) {
                ...
                const sixteenthFreq = (userSettings && userSettings.rhythmFrequencies && userSettings.rhythmFrequencies['16th'] !== undefined) ?
                    userSettings.rhythmFrequencies['16th'] : 20; // 默认20%

                console.log(`🎯 6/8拍十六分音符频率设置: ${sixteenthFreq}%`);
                ...
                if (sixteenthFreq > 0) {
                    ...
                    if (availableRhythms.includes('eighth')) {
                        patterns.push({
                            ...
                            weight: Math.round(sixteenthFreq * 0.6), // 🔥 修复：60%的频率权重
                            pattern: [
                        ...
                        console.log(`🎵 添加十六分音符+八分音符混合模式，权重: ${Math.round(sixteenthFreq * 0.6)}`);
                    }

                    // 模式2: 十二个十六分音符 - 较少出现，增加挑战性
                    patterns.push({
                        name: 'twelve16th',
                        weight: Math.round(sixteenthFreq * 0.15), // 🔥 修复：15%的频率权重
                        pattern: [
                            { duration: '16th', beats: 0.25 },
                            { duration: '16th', beats: 0.25 },
                            { duration: '16th', beats: 0.25 },
                    ...
                    console.log(`🎵 添加十二个十六分音符模式，权重: ${Math.round(sixteenthFreq * 0.15)}`);
                } else {
                    console.log(`🚫 十六分音符频率为0%，跳过十六分音符模式`);
                }
                
                // 模式3: 混合十六分音符和附点四分音符
                if (availableRhythms.includes('quarter.')) {
                    // 🔥 修复：使用十六分音符和附点四分音符的平均频率
                    const dottedQuarterFreq = (userSettings && userSettings.rhythmFrequencies && userSettings.rhythmFrequencies['dotted-quarter'] !== undefined) ?
                        userSettings.rhythmFrequencies['dotted-quarter'] : 35;
                    const mixedWeight = Math.round((sixteenthFreq + dottedQuarterFreq) / 4); // 较低权重，因为是混合模式

                    if (mixedWeight > 0) {
                        patterns.push({
                            name: 'sixteenth_dotted',
                            weight: mixedWeight, // 🔥 修复：使用计算的权重
                            pattern: [
                                { duration: '16th', beats: 0.25 },
                                { duration: '16th', beats: 0.25 },
                                { duration: '16th', beats: 0.25 },
                        ...
                        console.log(`🎵 添加十六分音符+附点四分音符混合模式，权重: ${mixedWeight}`);
                    } else {
                        console.log(`🚫 十六分音符或附点四分音符频率过低，跳过混合模式`);
                    }
                }
            }
...
${measuresXML}  </part>
        ...
        if (originalAllowedRhythms) {
            ...
            console.log(`🔄 6/8拍已恢复原始连音设置: [${originalAllowedRhythms.join(', ')}]`);
        }
    ...
    try {
        ...
        try {
            ...
            if (timeSignature === '6/8') {
                ...
                // 其他拍号：移除二连音和四连音，保留三连音
                const filteredOtherRhythms = originalAllowedRhythms.filter(rhythm =>
                    rhythm !== 'duplet' && rhythm !== 'quadruplet'
                );
                console.log(`🎵 ${timeSignature}拍连音过滤: 移除duplet/quadruplet，保留triplet`);
                console.log(`   原始: [${originalAllowedRhythms.join(', ')}]`);
                console.log(`   过滤后: [${filteredOtherRhythms.join(', ')}]`);
    ...
    if (typeof originalAllowedRhythms !== 'undefined') {
        userSettings.allowedRhythms = originalAllowedRhythms;
        console.log(`🔄 已恢复原始连音设置: [${originalAllowedRhythms.join(', ')}]`);
    }
...
function selectAllRhythms() {
    ...
    console.log(`🎵 全选节奏：当前可见的选项 [${visibleRhythmIds.join(', ')}]`);
...
function updateRhythmSettingsRealTime() {
    ...
    basicRhythmIds.forEach(id => {
        ...
        if (checkbox && checkbox.checked) {
            ...
            console.log(`✅ 基本节奏已选中: ${checkbox.value} (来自 ${id})`);
        }
    });
    ...
    // 🎯 如果开启了附点音符，为适合的基本节奏自动生成附点版本
    if (shouldAddDottedNotes) {
        const dottedToAdd = [];

        // 🎯 智能附点音符生成：只有当支持音符存在时才生成附点版本
        console.log(`🎵 智能附点音符生成开始，检查基础节奏支持...`);

        // 附点二分音符需要四分音符支持（附点二分音符 = 二分音符 + 四分音符）
        if (selectedRhythms.includes('half') && selectedRhythms.includes('quarter')) {
            if (!selectedRhythms.includes('half.')) {
                dottedToAdd.push('half.');
                console.log(`🎵 ✅ 添加附点二分音符 (基于二分音符+四分音符支持)`);
            }
        } else if (selectedRhythms.includes('half')) {
            console.log(`🎵 ⚠️ 跳过附点二分音符：需要四分音符支持但未勾选`);
        }

        // 附点四分音符需要八分音符支持（附点四分音符 = 四分音符 + 八分音符）
        if (selectedRhythms.includes('quarter') && selectedRhythms.includes('eighth')) {
            if (!selectedRhythms.includes('quarter.')) {
                dottedToAdd.push('quarter.');
                console.log(`🎵 ✅ 添加附点四分音符 (基于四分音符+八分音符支持)`);
            }
        } else if (selectedRhythms.includes('quarter')) {
            console.log(`🎵 ⚠️ 跳过附点四分音符：需要八分音符支持但未勾选`);
        }

        // 附点八分音符需要十六分音符支持（附点八分音符 = 八分音符 + 十六分音符）
        if (selectedRhythms.includes('eighth') && selectedRhythms.includes('16th')) {
            if (!selectedRhythms.includes('eighth.')) {
                dottedToAdd.push('eighth.');
                console.log(`🎵 ✅ 添加附点八分音符 (基于八分音符+十六分音符支持)`);
            }
        } else if (selectedRhythms.includes('eighth')) {
            console.log(`🎵 ⚠️ 跳过附点八分音符：需要十六分音符支持但未勾选`);
        }

        // 附点十六分音符需要三十二分音符支持，但通常不使用
        if (selectedRhythms.includes('16th') && selectedRhythms.includes('32nd')) {
            if (!selectedRhythms.includes('16th.')) {
                dottedToAdd.push('16th.');
                console.log(`🎵 ✅ 添加附点十六分音符 (基于十六分音符+三十二分音符支持)`);
            }
        }
    ...
    console.log(`📝 最终节奏设置: [${selectedRhythms.join(', ')}]`);
...
window.testScreenDimensions = function() {
    console.log('🧪 测试屏幕尺寸获取:');
    console.log(`  - window.innerWidth: ${window.innerWidth}`);
    console.log(`  - window.innerHeight: ${window.innerHeight}`);
    console.log('✅ 屏幕尺寸获取正常');
};

window.testGenerateMelodyExists = function() {
    console.log('🧪 测试generateMelody函数存在性:');
    ...
    if (typeof generateMelody === 'function') {
        console.log('✅ generateMelody函数存在且可用');
    } else {
        console.log('❌ generateMelody函数不存在');
    }
};
...
window.deepDebugAccidentals = function() {
    ...
    // 测试关键调号和音符组合
    const testCases = [
        { keySignature: 'C', midi: 60, noteName: 'C' },      // C大调中的C
        { keySignature: 'C', midi: 62, noteName: 'D' },      // C大调中的D
        { keySignature: 'G', midi: 62, noteName: 'D' },      // G大调中的D
        { keySignature: 'F', midi: 62, noteName: 'D' },      // F大调中的D
        { keySignature: 'Bb', midi: 62, noteName: 'D' },     // Bb大调中的D
    ];

    testCases.forEach(testCase => {
        console.log(`\n📝 测试: ${testCase.keySignature}大调中的${testCase.noteName}音符 (MIDI ${testCase.midi})`);
        ...
        console.log(`  结果统计（20次生成）:`);
        console.log(`    升号: ${sharps}次 (${(sharps/20*100).toFixed(1)}%)`);
        console.log(`    降号: ${flats}次 (${(flats/20*100).toFixed(1)}%)`);
        console.log(`    无变化: ${unchanged}次 (${(unchanged/20*100).toFixed(1)}%)`);

        if (sharps === 0) console.log(`    ❌ 升号生成被阻止`);
        if (flats === 0) console.log(`    ❌ 降号生成被阻止`);
        if (sharps > 0 && flats > 0) console.log(`    ✅ 升降号都能生成`);
    });
...
window.testUserReportedIssue = function() {
    ...
    function testAccidentalInKey(keySignature, testMidi, noteName) {
        console.log(`\n📍 测试 ${keySignature}大调中的${noteName}音符 (MIDI ${testMidi})`);

        const keyInfo = KEY_SIGNATURES[keySignature];
        const noteClass = testMidi % 12;
        const originalKeyInfo = isNoteAffectedByKeySignature(noteClass, keySignature);

        console.log(`  调号信息: 升${keyInfo.sharps.length}个, 降${keyInfo.flats.length}个`);
        console.log(`  原音符影响: ${JSON.stringify(originalKeyInfo)}`);
        ...
        console.log(`  升号测试: ${testMidi} → ${sharpNote} (类别${sharpNoteClass})`);
        console.log(`    升号音符被调号影响: ${JSON.stringify(sharpKeyInfo)}`);
        console.log(`    可以添加升号: ${canAddSharp}`);
        ...
        console.log(`  降号测试: ${testMidi} → ${flatNote} (类别${flatNoteClass})`);
        console.log(`    降号音符被调号影响: ${JSON.stringify(flatKeyInfo)}`);
        console.log(`    可以添加降号: ${canAddFlat}`);

        // 结论
        if (canAddSharp && canAddFlat) {
            console.log(`  ✅ 结论: 升号和降号都可以生成`);
        } else if (canAddSharp && !canAddFlat) {
            console.log(`  ❌ 结论: 只能生成升号，不能生成降号`);
        } else if (!canAddSharp && canAddFlat) {
            console.log(`  ❌ 结论: 只能生成降号，不能生成升号`);
        } else {
            console.log(`  ❌ 结论: 升号和降号都不能生成`);
        }
    ...
    // 测试用户描述的具体场景
    console.log('\n=== 测试C大调（无升降号）===');
    testAccidentalInKey('C', 60, 'C');  // C音符
    testAccidentalInKey('C', 62, 'D');  // D音符

    console.log('\n=== 测试G大调（升记号调号）===');
    testAccidentalInKey('G', 60, 'C');  // C音符
    testAccidentalInKey('G', 62, 'D');  // D音符

    console.log('\n=== 测试F大调（降记号调号）===');
    testAccidentalInKey('F', 60, 'C');  // C音符
    testAccidentalInKey('F', 62, 'D');  // D音符
...
window.showNoteClassMapping = function() {
    ...
    for (let i = 0; i < 12; i++) {
        console.log(`  类别 ${i}: ${noteNames[i]}`);
    }

    console.log('\n📊 重要调号的升降号类别:');
    console.log('  C大调: 无升降号');
    console.log('  G大调: F# (类别6)');
    console.log('  F大调: Bb (类别10)');
};

// 🧪 测试修复后的临时记号生成是否还有优先权问题
window.testAccidentalPriority = function() {
    console.log('\n🔬=== 临时记号优先权测试 ===');

    function testKeySignature(keySignature, testMidi, testName) {
        console.log(`\n📋 测试调号: ${keySignature}大调，音符: ${testName} (MIDI ${testMidi})`);
        ...
        console.log(`  🎼 4/4拍 ${totalTests}次测试:`);
        console.log(`    升号: ${results44.sharps}次 (${(results44.sharps/totalTests*100).toFixed(1)}%)`);
        console.log(`    降号: ${results44.flats}次 (${(results44.flats/totalTests*100).toFixed(1)}%)`);
        console.log(`    无变化: ${results44.unchanged}次 (${(results44.unchanged/totalTests*100).toFixed(1)}%)`);

        // 分析是否有明显偏向
        const ratio = results44.sharps / (results44.flats || 1);
        if (ratio > 2) {
            console.log(`    ⚠️  明显偏向升号 (比例 ${ratio.toFixed(1)}:1)`);
        } else if (ratio < 0.5) {
            console.log(`    ⚠️  明显偏向降号 (比例 1:${(1/ratio).toFixed(1)})`);
        } else if (results44.sharps > 0 && results44.flats > 0) {
            console.log(`    ✅ 升降号分布正常 (比例 ${ratio.toFixed(1)}:1)`);
        }
    ...
    // C大调（无升降号）
    results.push({ key: 'C', ...testKeySignature('C', 60, 'C音符') });
    results.push({ key: 'C', ...testKeySignature('C', 62, 'D音符') });

    // G大调（升记号调号）
    results.push({ key: 'G', ...testKeySignature('G', 60, 'C音符') });
    results.push({ key: 'G', ...testKeySignature('G', 62, 'D音符') });

    // F大调（降记号调号）
    results.push({ key: 'F', ...testKeySignature('F', 60, 'C音符') });
    results.push({ key: 'F', ...testKeySignature('F', 62, 'D音符') });
    ...
    console.log(`升记号调号平均比例: ${avgSharpKeyRatio.toFixed(2)}:1`);
    console.log(`降记号调号平均比例: ${avgFlatKeyRatio.toFixed(2)}:1`);
    console.log(`中性调号平均比例: ${avgNeutralRatio.toFixed(2)}:1`);

    if (avgSharpKeyRatio > 1.5) {
        console.log(`❌ 发现问题: 升记号调号中升号有明显优先权!`);
    }
    if (avgFlatKeyRatio < 0.67) {
        console.log(`❌ 发现问题: 降记号调号中降号有明显优先权!`);
    }
    if (Math.abs(avgNeutralRatio - 1) > 0.3) {
        console.log(`❌ 发现问题: 中性调号中选择不均匀!`);
    }
...
window.quickAccidentalTest = function() {
    ...
    try {
        ...
        console.log(`  升号(C#): ${results.sharps}次`);
        console.log(`  降号(Cb): ${results.flats}次`);
        console.log(`  无变化: ${results.unchanged}次`);

        if (results.sharps > 0 && results.flats > 0) {
            console.log('  ✅ 成功: 升号和降号都能生成!');
            return true;
        } else if (results.sharps > 0 && results.flats === 0) {
            console.log('  ❌ 问题仍存在: 只能生成升号，无法生成降号');
            return false;
        } else if (results.sharps === 0 && results.flats > 0) {
            console.log('  ❌ 问题仍存在: 只能生成降号，无法生成升号');
            return false;
        } else {
            console.log('  ❌ 严重问题: 无法生成任何临时记号');
            return false;
        }

    } catch (error) {
        console.log('  ❌ 测试发生错误:', error.message);
        return false;
    }
};
...
window.addAccidentalWithLogging = function(midi, keySignature) {
    console.log(`\n🧪 详细日志: 为MIDI ${midi}在${keySignature}大调中添加临时记号`);
    ...
    console.log(`  原音符: MIDI ${midi} → 类别 ${noteClass} (${noteNames[noteClass]})`);
    console.log(`  原音符受调号影响: ${JSON.stringify(keySignatureInfo)}`);
    ...
    console.log(`  升号测试: MIDI ${sharpNote} → 类别 ${sharpNoteClass} (${noteNames[sharpNoteClass]})`);
    console.log(`    升号音符受调号影响: ${JSON.stringify(sharpKeyInfo)}`);
    console.log(`    升号条件: ${sharpNote} <= 72 (${sharpNote <= 72}) && !isFlat (${!sharpKeyInfo.isFlat})`);

    if (sharpNote <= 72 && !sharpKeyInfo.isFlat) {
        accidentalChoices.push(sharpNote);
        console.log(`    ✅ 升号被添加到选择列表`);
    } else {
        console.log(`    ❌ 升号被拒绝`);
        if (sharpNote > 72) console.log(`      原因: 超出音域上限`);
        if (sharpKeyInfo.isFlat) console.log(`      原因: 升号后的音符已被调号降低`);
    }
    ...
    console.log(`  降号测试: MIDI ${flatNote} → 类别 ${flatNoteClass} (${noteNames[flatNoteClass]})`);
    console.log(`    降号音符受调号影响: ${JSON.stringify(flatKeyInfo)}`);
    console.log(`    降号条件: ${flatNote} >= 60 (${flatNote >= 60}) && !isSharp (${!flatKeyInfo.isSharp})`);

    if (flatNote >= 60 && !flatKeyInfo.isSharp) {
        accidentalChoices.push(flatNote);
        console.log(`    ✅ 降号被添加到选择列表`);
    } else {
        console.log(`    ❌ 降号被拒绝`);
        if (flatNote < 60) console.log(`      原因: 超出音域下限`);
        if (flatKeyInfo.isSharp) console.log(`      原因: 降号后的音符已被调号升高`);
    }

    console.log(`  最终可选临时记号: [${accidentalChoices.join(', ')}]`);

    if (accidentalChoices.length === 0) {
        console.log(`  🔄 结果: 无可用临时记号，返回原音符 ${midi}`);
        return midi;
    }

    const selected = accidentalChoices[Math.floor(Math.random() * accidentalChoices.length)];
    console.log(`  🎯 结果: 随机选择 ${selected}`);
    return selected;
};
...
window.finalAccidentalTest = function() {
    console.log('\n🚀 终极验证：测试临时记号修复结果');
    console.log('这个测试将验证用户报告的具体问题是否已解决\n');

    // 直接模拟用户场景
    console.log('=== 场景1: C大调（无升降号调号）===');
    console.log('❓ 问题：应该能生成升号和降号，不应该只有升号和还原记号');

    addAccidentalWithLogging(60, 'C');  // C音符
    addAccidentalWithLogging(62, 'C');  // D音符

    console.log('\n=== 场景2: G大调（升记号调号）===');
    console.log('❓ 问题：应该能生成升号和降号，不应该只有升号和还原记号');

    addAccidentalWithLogging(60, 'G');  // C音符
    addAccidentalWithLogging(62, 'G');  // D音符

    console.log('\n=== 场景3: F大调（降记号调号）===');
    console.log('❓ 问题：应该能生成升号和降号，不应该只有降号和还原记号');

    addAccidentalWithLogging(60, 'F');  // C音符
    addAccidentalWithLogging(62, 'F');  // D音符

    console.log('\n🎯 如果修复成功，您应该看到:');
    console.log('  - 在所有调号中，升号和降号都被添加到选择列表');
    console.log('  - 临时记号选择不再受调号类型限制');
    console.log('  - C大调、G大调、F大调都能生成完整的升降号组合');
...
window.deepDiagnoseAccidentalIssue = function() {
    ...
    testKeys.forEach(key => {
        ...
        console.log(`  ${key}大调: 升号[${keyInfo.sharps.join(',')}] 降号[${keyInfo.flats.join(',')}]`);
    });

    // 步骤2：检查isNoteAffectedByKeySignature函数
    console.log('\n🔍 步骤2：检查音符受调号影响检测');
    [60, 61, 62, 63].forEach(midi => {
        const noteClass = midi % 12;
        console.log(`\n  MIDI ${midi} (类别${noteClass}):`);
        ['C', 'G', 'F'].forEach(key => {
            const affected = isNoteAffectedByKeySignature(noteClass, key);
            console.log(`    ${key}大调: ${JSON.stringify(affected)}`);
        });
    });
    ...
    function simulateAccidentalGeneration(midi, keySignature) {
        console.log(`\n  📍 模拟: MIDI ${midi} 在 ${keySignature}大调`);

        const choices = [];
        const noteClass = midi % 12;
        const origInfo = isNoteAffectedByKeySignature(noteClass, keySignature);

        console.log(`    原音符类别${noteClass}: ${JSON.stringify(origInfo)}`);
        ...
        console.log(`    升号测试: ${midi}→${sharpNote} (类别${sharpClass})`);
        console.log(`      调号影响: ${JSON.stringify(sharpInfo)}`);
        console.log(`      条件: !isFlat = ${!sharpInfo.isFlat}`);
        console.log(`      结果: ${canSharp ? '✅可以' : '❌不可以'}`);

        if (canSharp) choices.push(`${sharpNote}(升号)`);
        ...
        console.log(`    降号测试: ${midi}→${flatNote} (类别${flatClass})`);
        console.log(`      调号影响: ${JSON.stringify(flatInfo)}`);
        console.log(`      条件: !isSharp = ${!flatInfo.isSharp}`);
        console.log(`      结果: ${canFlat ? '✅可以' : '❌不可以'}`);

        if (canFlat) choices.push(`${flatNote}(降号)`);

        console.log(`    最终选择: [${choices.join(', ')}]`);
    ...
    console.log('\n  === C大调测试 ===');
    simulateAccidentalGeneration(60, 'C');  // C音符
    simulateAccidentalGeneration(62, 'C');  // D音符

    console.log('\n  === G大调测试 ===');
    simulateAccidentalGeneration(60, 'G');  // C音符
    simulateAccidentalGeneration(62, 'G');  // D音符

    console.log('\n  === F大调测试 ===');
    simulateAccidentalGeneration(60, 'F');  // C音符
    simulateAccidentalGeneration(62, 'F');  // D音符

    // 步骤4：检查是否有其他隐藏的限制
    console.log('\n🕵️ 步骤4：检查可能的其他限制');
    console.log('  - 检查音域限制 (60-72): ✅');
    console.log('  - 检查同音异名冲突避免: ✅');
    console.log('  - 检查调号类型限制: 🔍需要验证');
...
window.realTimeAccidentalTest = function() {
    ...
    function testRealGeneration(keySignature, testMidi, testName) {
        console.log(`\n🎼 测试 ${keySignature}大调中的${testName} (MIDI ${testMidi})`);
        ...
        console.log(`  📊 ${totalTests}次测试结果:`);
        console.log(`    升号: ${results.sharps}次 (${(results.sharps/totalTests*100).toFixed(1)}%)`);
        console.log(`    降号: ${results.flats}次 (${(results.flats/totalTests*100).toFixed(1)}%)`);
        console.log(`    无变化: ${results.unchanged}次 (${(results.unchanged/totalTests*100).toFixed(1)}%)`);

        // 分析结果
        if (results.sharps > 0 && results.flats > 0) {
            console.log(`    ✅ 结论: 升号和降号都能正常生成`);
        } else if (results.sharps > 0 && results.flats === 0) {
            console.log(`    ❌ 问题: 只能生成升号，无法生成降号`);
        } else if (results.sharps === 0 && results.flats > 0) {
            console.log(`    ❌ 问题: 只能生成降号，无法生成升号`);
        } else {
            console.log(`    ❌ 严重问题: 无法生成任何临时记号`);
        }
    ...
    // 测试用户报告的关键场景
    console.log('\n🎯 用户关键场景测试:');
    testRealGeneration('C', 60, 'C音符');
    testRealGeneration('C', 62, 'D音符');
    testRealGeneration('G', 60, 'C音符');
    testRealGeneration('G', 62, 'D音符');
    testRealGeneration('F', 60, 'C音符');
    testRealGeneration('F', 62, 'D音符');
...
window.test68AccidentalGeneration = function() {
    ...
    function test68Path(pathName, midi, keySignature) {
        console.log(`\n🎵 测试${pathName} - ${keySignature}大调 MIDI ${midi}`);
        ...
        if (pathName === '主路径') {
            ...
            console.log(`    可选临时记号: [${choices.join(', ')}]`);

            const hasSharp = choices.includes(midi + 1);
            const hasFlat = choices.includes(midi - 1);
            console.log(`    升号可用: ${hasSharp ? '✅' : '❌'}`);
            console.log(`    降号可用: ${hasFlat ? '✅' : '❌'}`);

        } else if (pathName === 'Beat Clarity路径') {
            // 测试Beat Clarity路径 - 需要找到该函数
            console.log(`    ⏳ Beat Clarity路径测试待实现`);
        }
    }
...
window.finalConfirmation = function() {
    ...
    function checkCurrentLogic(midi, keySignature) {
        console.log(`\n🔍 检查 ${keySignature}大调中MIDI ${midi}的逻辑:`);
        ...
        console.log(`  升号检查: ${midi} → ${sharpNote}`);
        console.log(`    升号音符类别 ${sharpClass} 受调号影响: ${JSON.stringify(sharpInfo)}`);
        console.log(`    条件: !isFlat = ${!sharpInfo.isFlat}`);
        console.log(`    结果: ${!sharpInfo.isFlat ? '✅允许' : '❌禁止'}`);
        ...
        console.log(`  降号检查: ${midi} → ${flatNote}`);
        console.log(`    降号音符类别 ${flatClass} 受调号影响: ${JSON.stringify(flatInfo)}`);
        console.log(`    条件: !isSharp = ${!flatInfo.isSharp}`);
        console.log(`    结果: ${!flatInfo.isSharp ? '✅允许' : '❌禁止'}`);
    ...
    const testCases = [
        { key: 'C', midi: 60, name: 'C大调中的C音符' },
        { key: 'C', midi: 62, name: 'C大调中的D音符' },
        { key: 'G', midi: 60, name: 'G大调中的C音符' },
        { key: 'G', midi: 62, name: 'G大调中的D音符' },
        { key: 'F', midi: 60, name: 'F大调中的C音符' },
        { key: 'F', midi: 62, name: 'F大调中的D音符' }
    ];
    ...
    results.forEach(result => {
        const status = result.canSharp && result.canFlat ? '✅修复成功' : '❌仍有问题';
        console.log(`  ${result.name}: ${status}`);
        if (!result.canSharp || !result.canFlat) {
            allFixed = false;
            if (!result.canSharp) console.log(`    - 升号被阻止`);
            if (!result.canFlat) console.log(`    - 降号被阻止`);
        }
    });
    ...
    console.log('\n🎯 步骤4：最终结论');
    if (allFixed) {
        console.log('✅ 修复成功！所有测试场景都能正常生成升号和降号');
        console.log('   用户报告的问题应该已经解决');
    } else {
        console.log('❌ 修复不完整！仍有场景无法正常生成临时记号');
        console.log('   需要进一步调查和修复');
    }
...
window.verifyRhythmDefaults = function() {
    ...
    // 检查userSettings中的默认设置
    console.log('\n📋 步骤1：检查userSettings默认设置');
    console.log(`  当前allowedRhythms: [${userSettings.allowedRhythms.join(', ')}]`);
    ...
    if (isCorrect) {
        console.log('  ✅ JavaScript默认设置正确');
    } else {
        console.log('  ❌ JavaScript默认设置不正确');
        console.log(`    期望: [${expectedRhythms.join(', ')}]`);
        console.log(`    实际: [${actualRhythms.join(', ')}]`);
    }

    // 检查HTML中的checkbox状态
    console.log('\n📋 步骤2：检查HTML中的checkbox默认状态');

    const checkboxes = [
        { id: 'rhythm-half', name: '二分音符', shouldBeChecked: true },
        { id: 'rhythm-quarter', name: '四分音符', shouldBeChecked: true },
        { id: 'rhythm-eighth', name: '八分音符', shouldBeChecked: true },
        { id: 'rhythm-16th', name: '十六分音符', shouldBeChecked: false }
    ];
    ...
    checkboxes.forEach(checkbox => {
        ...
        if (element) {
            ...
            const status = actualState === expectedState ? '✅' : '❌';

            console.log(`  ${status} ${checkbox.name}: ${actualState ? '勾选' : '未勾选'} (期望: ${expectedState ? '勾选' : '未勾选'})`);

            if (actualState !== expectedState) {
                htmlCorrect = false;
            }
        } else {
            console.log(`  ⚠️  找不到${checkbox.name}的checkbox元素`);
        }
    });
    ...
    if (overallCorrect) {
        console.log('✅ 修复成功！节奏默认设置完全正确');
        console.log('   - JavaScript默认只包含二分音符、四分音符、八分音符');
        console.log('   - HTML中十六分音符默认不勾选');
        console.log('   - 用户初次打开页面将看到期望的默认节奏设置');
    } else {
        console.log('❌ 修复不完整，仍需调整');
        if (!isCorrect) console.log('   - JavaScript默认设置需要修正');
        if (!htmlCorrect) console.log('   - HTML checkbox状态需要修正');
    }
...
window.analyzeEnharmonicIssue = function() {
    ...
    ['C', 'G', 'F'].forEach(key => {
        const keyInfo = KEY_SIGNATURES[key];
        console.log(`  ${key}大调:`);
        console.log(`    升号类别: [${keyInfo.sharps.join(', ')}] (${keyInfo.sharps.map(s => noteClassNames[s]).join(', ')})`);
        console.log(`    降号类别: [${keyInfo.flats.join(', ')}] (${keyInfo.flats.map(f => noteClassNames[f]).join(', ')})`);
    });

    // 步骤2：详细分析isNoteAffectedByKeySignature函数
    console.log('\n🔍 步骤2：分析isNoteAffectedByKeySignature函数行为');

    function detailedNoteAnalysis(noteClass, keySignature) {
        console.log(`\n  📍 分析音符类别 ${noteClass} (${noteClassNames[noteClass]}) 在 ${keySignature}大调中:`);

        const keyInfo = KEY_SIGNATURES[keySignature];
        const result = isNoteAffectedByKeySignature(noteClass, keySignature);

        console.log(`    调号信息: 升号[${keyInfo.sharps.join(',')}] 降号[${keyInfo.flats.join(',')}]`);
        console.log(`    检测结果: ${JSON.stringify(result)}`);
        console.log(`    是否被升高: ${result.isSharp}`);
        console.log(`    是否被降低: ${result.isFlat}`);
        ...
        if (result.isSharp !== shouldBeSharp) {
            console.log(`    ❌ 升号检测错误: 期望${shouldBeSharp}, 实际${result.isSharp}`);
        }
        if (result.isFlat !== shouldBeFlat) {
            console.log(`    ❌ 降号检测错误: 期望${shouldBeFlat}, 实际${result.isFlat}`);
        }
    ...
    // 测试关键音符类别
    const testCases = [
        { noteClass: 0, keySignature: 'C' },  // C在C大调
        { noteClass: 1, keySignature: 'C' },  // C#/Db在C大调
        { noteClass: 2, keySignature: 'C' },  // D在C大调
        { noteClass: 0, keySignature: 'G' },  // C在G大调
        { noteClass: 1, keySignature: 'G' },  // C#/Db在G大调
        { noteClass: 6, keySignature: 'G' },  // F#/Gb在G大调
        { noteClass: 0, keySignature: 'F' },  // C在F大调
        { noteClass: 10, keySignature: 'F' }, // A#/Bb在F大调
    ];
    ...
    // 步骤3：分析临时记号生成的具体阻塞点
    console.log('\n⚙️ 步骤3：分析临时记号生成的具体阻塞点');

    function analyzeAccidentalBlocking(originalMidi, keySignature) {
        const originalClass = originalMidi % 12;
        console.log(`\n  🎯 分析MIDI ${originalMidi} (类别${originalClass}, ${noteClassNames[originalClass]}) 在${keySignature}大调:`);
        ...
        console.log(`    升号分析: ${originalMidi} → ${sharpMidi} (类别${sharpClass}, ${noteClassNames[sharpClass]})`);
        console.log(`      升号音符受调号影响: ${JSON.stringify(sharpInfo)}`);
        console.log(`      阻塞条件: isFlat = ${sharpInfo.isFlat}`);
        console.log(`      升号${sharpInfo.isFlat ? '❌被阻塞' : '✅可生成'}`);
        ...
        console.log(`    降号分析: ${originalMidi} → ${flatMidi} (类别${flatClass}, ${noteClassNames[flatClass]})`);
        console.log(`      降号音符受调号影响: ${JSON.stringify(flatInfo)}`);
        console.log(`      阻塞条件: isSharp = ${flatInfo.isSharp}`);
        console.log(`      降号${flatInfo.isSharp ? '❌被阻塞' : '✅可生成'}`);
    ...
    // 测试用户报告的具体问题
    const problemCases = [
        { midi: 60, key: 'C', name: 'C大调中的C音符' },
        { midi: 62, key: 'C', name: 'C大调中的D音符' },
        { midi: 60, key: 'G', name: 'G大调中的C音符' },
        { midi: 62, key: 'G', name: 'G大调中的D音符' },
        { midi: 60, key: 'F', name: 'F大调中的C音符' },
        { midi: 62, key: 'F', name: 'F大调中的D音符' }
    ];
    ...
    if (foundIssues.length > 0) {
        console.log('❌ 发现同音异名检测问题:');
        foundIssues.forEach(issue => {
            console.log(`  ${issue.name}:`);
            if (issue.sharpBlocked) console.log(`    - 升号被错误阻塞`);
            if (issue.flatBlocked) console.log(`    - 降号被错误阻塞`);
        });
    } else {
        console.log('✅ 同音异名检测逻辑看起来正常');
    }
...
window.testSimplifiedAccidentalLogic = function() {
    ...
    function simplifiedAddAccidental(midi, keySignature, range = { min: 60, max: 79 }) {
        ...
        if (sharpNote <= range.max) {
            accidentalChoices.push(sharpNote);
            console.log(`  ✅ 升号可用: ${midi} → ${sharpNote}`);
        } else {
            console.log(`  ❌ 升号超出音域: ${sharpNote} > ${range.max}`);
        }
        ...
        if (flatNote >= range.min) {
            accidentalChoices.push(flatNote);
            console.log(`  ✅ 降号可用: ${midi} → ${flatNote}`);
        } else {
            console.log(`  ❌ 降号超出音域: ${flatNote} < ${range.min}`);
        }

        console.log(`  可选临时记号: [${accidentalChoices.join(', ')}]`);
    ...
    testCases.forEach(testCase => {
        ...
        console.log(`    原始逻辑可选: [${originalChoices.join(', ')}]`);
        ...
        console.log('  📈 对比分析:');
        console.log(`    升号: 原始${originalHasSharp ? '✅' : '❌'} vs 简化${simplifiedHasSharp ? '✅' : '❌'}`);
        console.log(`    降号: 原始${originalHasFlat ? '✅' : '❌'} vs 简化${simplifiedHasFlat ? '✅' : '❌'}`);

        if (simplifiedHasSharp && simplifiedHasFlat) {
            if (!originalHasSharp || !originalHasFlat) {
                console.log(`    🚨 同音异名检测阻止了合理的临时记号生成！`);
            }
        }
    });

    console.log('\n🎯 结论');
    console.log('如果简化逻辑显示所有情况都能生成升号和降号，');
    console.log('而原始逻辑有阻塞，则说明同音异名检测确实是问题所在。');
...
window.fixEnharmonicLogic = function() {
    ...
    console.log('当前临时记号生成使用的是严格的同音异名冲突检测：');
    console.log('- 如果升号后的音符被调号降低，则禁止生成升号');
    console.log('- 如果降号后的音符被调号升高，则禁止生成降号');
    console.log('');
    console.log('但这种检测可能过于严格，实际上：');
    console.log('- 临时记号的目的就是创造调外音');
    console.log('- 同音异名在不同上下文中是可以接受的');
    console.log('');
    console.log('建议的修复方案：');
    console.log('1. 完全移除同音异名冲突检测');
    console.log('2. 或者使用更宽松的检测逻辑');
    console.log('3. 只保留音域检查');
    ...
    window.addAccidentalFixed = function(midi, keySignature, range = { min: 60, max: 79 }) {
        ...
        if (accidentalChoices.length > 0) {
            const selected = accidentalChoices[Math.floor(Math.random() * accidentalChoices.length)];
            console.log(`🎯 修复版本: MIDI ${midi} → ${selected} 在 ${keySignature}大调`);
            return selected;
        }

        console.log(`🎯 修复版本: MIDI ${midi} → ${midi} (无变化) 在 ${keySignature}大调`);
        return midi;
    };

    console.log('✅ 修复版本函数已创建: window.addAccidentalFixed()');
    console.log('');
    console.log('您可以使用以下代码测试修复版本：');
    console.log('addAccidentalFixed(60, "C")  // C大调中的C音符');
    console.log('addAccidentalFixed(62, "G")  // G大调中的D音符');
    console.log('addAccidentalFixed(60, "F")  // F大调中的C音符');
...
function generate68MeasureWithBeatClarity(measureNumber, currentMidi, scale, userRange, maxJump, isLastMeasure, random, needsPhraseBreathe = false, clef = 'treble', keySignature = 'C', accidentalRate = 0) {
    ...
    console.log(`  - 四连音: ${hasQuadruplet} && quadruplet_freq=${userSettings?.rhythmFrequencies?.quadruplet}% -> ${isQuadrupletAllowed}`);
        ...
        const patternWeights = availablePatterns.map(pattern => {
            ...
            // 🎯 八分音符模式：应用频率控制
                ...
                if (userSettings?.rhythmFrequencies?.eighth !== undefined) {
                    ...
                    if (userFreq === 0) {
                        ...
                        // 🔥 修复：使用直接映射，频率等于权重
                        weight = userFreq;
                        console.log(`🎯 八分音符频率控制（直接映射）：${userFreq}% -> 权重${weight}`);
                    }
                } else {
                    weight = 15; // 默认权重
            ...
            // 🎶 十六分音符模式：中等权重（使用用户频率设置）
                ...
                if (userSettings && userSettings.rhythmFrequencies && userSettings.rhythmFrequencies['16th'] !== undefined) {
                    ...
                    if (userFreq === 0) {
                        ...
                        weight = userFreq; // 频率直接等于权重（频率100% = 权重30）
            ...
            // 🎶 二连音模式：使用用户频率设置
                ...
                if (userSettings && userSettings.rhythmFrequencies && userSettings.rhythmFrequencies.duplet !== undefined) {
                    ...
                    if (userFreq === 0) {
                        console.log(`🚫 6/8拍用户频率设置：二连音频率${userFreq}%，权重设为0`);
                        weight = 0;
                    } else {
                        weight = userFreq; // 频率直接等于权重
                        console.log(`🎯 6/8拍用户频率设置：二连音 = ${userFreq}%，权重 = ${weight}`);
                    }
                } else {
            ...
            // 🎹 四连音模式：使用用户频率设置
                ...
                if (userSettings && userSettings.rhythmFrequencies && userSettings.rhythmFrequencies.quadruplet !== undefined) {
                    ...
                    if (userFreq === 0) {
                        console.log(`🚫 6/8拍用户频率设置：四连音频率${userFreq}%，权重设为0`);
                        weight = 0;
                    } else {
                        weight = userFreq; // 频率直接等于权重
                        console.log(`🎯 6/8拍用户频率设置：四连音 = ${userFreq}%，权重 = ${weight}`);
                    }
                } else {
        ...
        if (userRhythms.includes('eighth')) {
            ...
            if (userSettings && userSettings.rhythmFrequencies && userSettings.rhythmFrequencies.eighth !== undefined) {
                ...
                if (userFreq === 0) {
                    ...
                    eighthWeight = userFreq; // 频率直接等于权重
        ...
        if (userRhythms.includes('quarter') && userRhythms.includes('eighth')) {
            ...
            if (userSettings && userSettings.rhythmFrequencies) {
                ...
                if (quarterFreq === 0 || eighthFreq === 0) {
                    ...
                    console.log(`🎯 6/8拍混合模式频率：平均值 (${quarterFreq}% + ${eighthFreq}%) / 2 = ${mixedWeight}`);
                }
            }
        ...
        if (userRhythms.includes('duplet')) {
            ...
            if (userSettings && userSettings.rhythmFrequencies && userSettings.rhythmFrequencies.duplet !== undefined) {
                ...
                if (userFreq === 0) {
                    ...
                    dupletWeight = userFreq; // 频率直接等于权重
        ...
        if (userRhythms.includes('quadruplet')) {
            ...
            if (userSettings && userSettings.rhythmFrequencies && userSettings.rhythmFrequencies.quadruplet !== undefined) {
                ...
                if (userFreq === 0) {
                    ...
                    quadrupletWeight = userFreq; // 频率直接等于权重
        ...
        if (userRhythms.includes('16th') || userRhythms.includes('sixteenth')) {
            ...
            if (userSettings && userSettings.rhythmFrequencies && userSettings.rhythmFrequencies['16th'] !== undefined) {
                ...
                if (userFreq === 0) {
                    ...
                    sixteenthWeight = userFreq; // 频率直接等于权重
        ...
        if (userRhythms.includes('dotted-half') || userRhythms.includes('half.')) {
            ...
            if (userSettings && userSettings.rhythmFrequencies && userSettings.rhythmFrequencies.whole !== undefined) {
                ...
                if (userFreq === 0) {
                    ...
                    dottedHalfWeight = userFreq; // 频率直接等于暓重
...
function updateRhythmOptionsForTimeSignature(timeSignature) {
    ...
    if (timeSignature === '6/8') {
        ...
        // 🔥 关键修复：显示所有节奏选项（三连音、二连音、四连音）
        // 当选择多个拍号时，显示所有可能的节奏选项，让用户自由选择
        ...
        console.log('🎵 多拍号模式：所有节奏选项已显示，包括三连音、二连音、四连音');
    } else {
        ...
        if (wholeLabel && dottedHalfLabel) {
            ...
            // 在4/4拍中隐藏附点二分音符选项，由通用的"附点音符"开关控制
        ...
        if (halfLabel && dottedQuarterLabel) {
            ...
            // 在4/4拍中隐藏附点四分音符选项，由通用的"附点音符"开关控制
            if (timeSignature === '4/4' || timeSignature === '2/4' || timeSignature === '3/4') {
        ...
        if (freqDottedHalfItem) {
            // 在4/4、2/4、3/4拍中隐藏附点二分音符频率控制，由通用的附点音符逻辑处理
            if (timeSignature === '4/4' || timeSignature === '2/4' || timeSignature === '3/4') {
        ...
        // 显示节奏频率控制，但在标准拍号中隐藏具体的附点音符频率控制
        if (freqHalfItem) {
            freqHalfItem.style.display = 'block';
        }
        if (freqDottedQuarterItem) {
            // 在4/4、2/4、3/4拍中隐藏附点四分音符频率控制，由通用的附点音符逻辑处理
            if (timeSignature === '4/4' || timeSignature === '2/4' || timeSignature === '3/4') {
        ...
        console.log(`🎵 ${timeSignature}拍模式：显示基本节奏选项，具体附点音符由通用"附点音符"开关控制`);
    }
...
function getDefaultRhythmFrequency(type) {
    const defaults = {
        ...
        'dotted-quarter': 35,
        quarter: 50,
        'dotted-eighth': 25,   // 🔥 添加缺失的dotted-eighth默认频率
        eighth: 40,
        '16th': 20,
        triplet: 15,
        duplet: 30,
...
function testAccidentalGeneration() {
    ...
    // 测试多次以观察随机性
    for (let i = 0; i < 10; i++) {
        const result = cMajorGenerator.addAccidental(60); // C音符
        if (result === 61) {
            console.log(`  ✅ 生成了升号: C → C#`);
        } else if (result === 59) {
            console.log(`  ✅ 生成了降号: C → B`);
        } else {
            console.log(`  ❌ 无变化: C → C`);
        }
    }
    ...
    for (let i = 0; i < 10; i++) {
        const result = gMajorGenerator.addAccidental(62); // D音符
        if (result === 63) {
            console.log(`  ✅ 生成了升号: D → D#`);
        } else if (result === 61) {
            console.log(`  ✅ 生成了降号: D → Db`);
        } else {
            console.log(`  ❌ 无变化: D → D`);
        }
    }
    ...
    for (let i = 0; i < 10; i++) {
        const result = fMajorGenerator.addAccidental(62); // D音符
        if (result === 63) {
            console.log(`  ✅ 生成了升号: D → D#`);
        } else if (result === 61) {
            console.log(`  ✅ 生成了降号: D → Db`);
...
function debugAddAccidental(midi, keySignature) {
    console.log(`\n🔍 调试临时记号生成: MIDI ${midi}, 调号 ${keySignature}`);

    const accidentalChoices = [];
    const noteClass = midi % 12;
    const keySignatureInfo = isNoteAffectedByKeySignature(noteClass, keySignature);

    console.log(`  原音符类别: ${noteClass}, 调号影响: ${JSON.stringify(keySignatureInfo)}`);
    ...
    console.log(`  升号测试: ${midi} + 1 = ${sharpNote} (类别 ${sharpNoteClass})`);
    console.log(`  升号音符调号影响: ${JSON.stringify(sharpKeyInfo)}`);
    console.log(`  升号条件: sharpNote <= 72 (${sharpNote <= 72}) && !sharpKeyInfo.isFlat (${!sharpKeyInfo.isFlat})`);

    if (sharpNote <= 72 && !sharpKeyInfo.isFlat) {
        accidentalChoices.push(sharpNote);
        console.log(`  ✅ 升号可用: ${sharpNote}`);
    } else {
        console.log(`  ❌ 升号不可用`);
    }
    ...
    console.log(`  降号测试: ${midi} - 1 = ${flatNote} (类别 ${flatNoteClass})`);
    console.log(`  降号音符调号影响: ${JSON.stringify(flatKeyInfo)}`);
    console.log(`  降号条件: flatNote >= 60 (${flatNote >= 60}) && !flatKeyInfo.isSharp (${!flatKeyInfo.isSharp})`);

    if (flatNote >= 60 && !flatKeyInfo.isSharp) {
        accidentalChoices.push(flatNote);
        console.log(`  ✅ 降号可用: ${flatNote}`);
    } else {
        console.log(`  ❌ 降号不可用`);
    }
    ...
    if (accidentalChoices.length === 0) {
        console.log(`  结果: 无临时记号，返回原音符 ${midi}`);
        return midi;
    }

    const selected = accidentalChoices[0]; // 选择第一个用于测试
    console.log(`  结果: 选择临时记号 ${selected}`);
    return selected;
}
    ...
    timeSignatureIds.forEach(id => {
        ...
        if (checkbox) {
            checkbox.addEventListener('change', function() {
                ...
                console.log(`🎵 拍号${this.value}勾选状态: ${this.checked}（将在保存时统一更新节奏选项）`);
            });
        } else {
...
function closeTimeSignatureSettingsWithSave() {
    ...
    if (selectedTimeSignatures.length > 0) {
        ...
        // 🎵 根据选择的拍号调整节奏选项（与saveTimeSignatureSettings保持一致）
        if (selectedTimeSignatures.length === 1) {
            ...
            // 🔥 修复：多拍号选择时显示所有节奏选项，包括三连音、二连音、四连音
            console.log('🎛️ 多拍号选择，显示所有节奏选项（包括三连音、二连音、四连音）');
            updateRhythmOptionsForTimeSignature('multi'); // 使用特殊的多拍号模式
        }
...
function saveTimeSignatureSettings() {
    ...
    try {
        ...
        // 更新按钮显示
        console.log('🔍 开始更新按钮显示');
        updateButtonDisplay('timeSignatureSettingsBtn', selectedTimeSignatures, '拍号');
        console.log('🔍 按钮显示更新完成');

        console.log('🔍 开始关闭设置弹窗');
        document.getElementById('timeSignatureModal').style.display = 'none';
        console.log('🔍 设置弹窗关闭完成');

    } catch (error) {
        console.error('❌ 保存拍号设置时出错:', error);
        alert(`保存拍号设置时出错: ${error.message}`);
    }
}
