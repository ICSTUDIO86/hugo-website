...
(function(){
  ...
  function getUnifiedTempoFromHeader(){
    ...
    const metronomeInput = document.getElementById('metronomeBpm') || document.getElementById('headerMetronomeBpm');
    return clampChallengeTempo(metronomeInput ? metronomeInput.value : 80);
  }
  ...
  function syncTempoAcrossInputs(rawTempo){
    ...
    if (typeof setMetronomeTempo === 'function') {
      ...
      const metronomeInput = document.getElementById('metronomeBpm') || document.getElementById('headerMetronomeBpm');
      if (metronomeInput) {
        metronomeInput.value = String(tempo);
      }
    }
    const challengeInput = $('challengeBPM');
    if (challengeInput && parseInt(challengeInput.value, 10) !== tempo) {
      isSyncingChallengeBpmInput = true;
      challengeInput.value = String(tempo);
  ...
  function computeJudgeWindowSec(bpm){
    ...
    const voiceMode = parseInt(getVoiceMode(), 10) || 1;
    if (voiceMode <= 1){
      const strict = beatSec * 0.12;
      return Math.max(0.038, Math.min(0.14, strict));
    }
    const win = beatSec * 0.18;
    return Math.max(0.06, Math.min(0.22, win));
  }
  ...
  function getJudgeWindowByIndex(index){
    ...
    if (index === 0) {
      if (voiceMode <= 1){
        return {
          early: Math.min(0.12, base + 0.035),
          late: Math.min(0.14, base + 0.04)
        };
      }
      return {
        early: Math.min(0.2, base + 0.08),
        late: Math.min(0.24, base + 0.1)
      };
    }
  ...
  function buildJudgeTimeline(bpm){
    ...
    for (const ev of sorted){
      ...
      const maxTargetXDistance = mr ? Math.max(12, mr.width * 0.24) : 26;
      const list = elementsByMeasure.get(ev.measure) || [];
      ...
      if (assignmentList.length){
        ...
        if (chosen && chosen.el && Number.isFinite(targetX) && Number.isFinite(chosen.x)) {
          const xDistance = Math.abs(chosen.x - targetX);
          if (xDistance > maxTargetXDistance) {
            chosen = null;
          }
        }
        if (chosen && chosen.el) {
  ...
  function handleCalibrationInput(voice){
    ...
    const getOverdueLate = (index, ev) => {
      ...
      const targetCount = Array.isArray(ev?.targets) ? ev.targets.length : 1;
      const extraFirst = index === 0 ? (voiceMode <= 1 ? 0.045 : 0.12) : 0;
      const extraChord = (voiceMode > 1 && targetCount > 1) ? 0.06 : 0;
      return Math.max(w.late + extraFirst + extraChord, index === 0 ? (voiceMode <= 1 ? 0.16 : 0.28) : w.late);
    };
    const advanceIndex = () => {
    ...
    const getEventWindowInfo = (index, ev) => {
      const w = getJudgeWindowByIndex(index);
      const targetCount = Array.isArray(ev?.targets) ? ev.targets.length : 1;
      let late = w.late + ((voiceMode > 1 && targetCount > 1) ? 0.06 : 0);
      if (index === 0) {
        late = Math.max(late + (voiceMode <= 1 ? 0.045 : 0.12), voiceMode <= 1 ? 0.16 : 0.28);
      }
    ...
    const canLookaheadToNext = (currentEv, nextEv) => {
      if (!currentEv || !nextEv || nextEv.judged) return false;
      if (currentEv.measure != null && nextEv.measure != null && nextEv.measure !== currentEv.measure) {
        return false;
      }
      const timeGap = nextEv.timeSec - currentEv.timeSec;
      if (isFinite(timeGap) && timeGap > 0.95) return false;
      const beatGap = nextEv.absBeats - currentEv.absBeats;
      if (isFinite(beatGap) && beatGap > 1.25) return false;
      return true;
    };
    ...
    if (currentWindow.inWindow && currentPick.match) {
      ...
      if (voiceMode > 1 && currentPick.hasVoicePending && canLookaheadToNext(currentEv, nextEv)) {
    ...
    if (voiceMode > 1 && !currentPick.hasVoicePending) {
      const nextIdx = currentIdx + 1;
      const nextEv = nextIdx < events.length ? events[nextIdx] : null;
      if (canLookaheadToNext(currentEv, nextEv)) {
        const nextWindow = getEventWindowInfo(nextIdx, nextEv);
        const nextPick = pickMatchForEvent(nextEv);
    ...
    } else if (voiceMode > 1 && currentPick.hasVoicePending) {
  ...
  function getChallengeSettings(){
    ...
    if (!modalOpen && saved){
      ...
      if (bpmEl) bpmEl.value = String(getUnifiedTempoFromHeader());
    ...
    if (bpmEl) settings.bpm = clampChallengeTempo((bpmEl.value || String(getUnifiedTempoFromHeader())));
    const cursorToggle = $('challengeCursorToggle');
    const metronomeToggle = $('challengeMetronomeToggle');
    const hideToggle = $('challengeHideToggle');
    const calibrationToggle = $('challengeCalibrationToggle');
  ...
  function openChallengeModal(){
    ...
    if (hideToggle && !hasPref) hideToggle.checked = false;
  ...
  if (metronomeBpmEl) {
    ...
    metronomeBpmEl.addEventListener('blur', onMetronomeBpmChanged);
  }
  syncTempoAcrossInputs(getUnifiedTempoFromHeader());
