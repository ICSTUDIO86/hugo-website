...
(function() {
  ...
  function genSign(params) {
    ...
    console.log('Form sign string:', signString);
  ...
  window.createZPayment = function() {
    console.log('🚀 Starting Z-Pay Form implementation...');
    ...
    try {
      ...
      form.method = 'POST';
      form.action = CONFIG.apiUrl; // 使用mapi.php
      form.style.display = 'none';
      form.enctype = 'multipart/form-data'; // 确保form-data格式
      ...
      modal.innerHTML = `
        ...
        <div style="font-size: 22px; font-weight: 600; margin-bottom: 15px;">正在跳转支付页面</div>
        <div style="font-size: 16px; opacity: 0.9; margin-bottom: 20px;">使用表单提交方式，避免跨域问题</div>
        <div style="width: 200px; height: 4px; background: rgba(255,255,255,0.3); border-radius: 2px; margin: 20px auto;">
          <div style="width: 100%; height: 100%; background: white; border-radius: 2px; animation: pulse 1.5s ease-in-out infinite;"></div>
        </div>
      `;
      
      // 添加CSS动画
      if (!document.getElementById('zpay-form-styles')) {
        const style = document.createElement('style');
        style.id = 'zpay-form-styles';
        style.textContent = `
          @keyframes pulse {
            0%, 100% { opacity: 0.5; }
            50% { opacity: 1; }
      ...
      console.error('❌ Z-Pay Form error:', error);
      alert(`支付创建失败: ${error.message}\n请刷新页面重试`);
    }
  };

  console.log('✅ Z-Pay Form implementation loaded');
  console.log('📋 Using form submission to avoid CORS issues');
